...
function NameGetFocus(){
    document.frmLogin.admin_name.focus();
}
function CheckValid(){
    if(document.frmLogin.admin_name.value=="您的昵称"){
        alert("请输入您的昵称");
        document.frmLogin.admin_name.focus();
        return false;
    }
    if(document.frmLogin.admin_pass.value==""){
        alert("请输入您的密码");
...
<body  style="background: url('imgs/bg.jpg'); ">
            	...
            	<div class="myform col-lg-6 animated" data-animation="flipInX" data-animation-delay="600" >
		         	 <form class="" name="frmLogin" action="check_user.php" method="post">	
		         	 <h3>管理员登录</h3>						
						<label for="name">您的姓名</label>
						<input id="name" class="form-control" type="text" required="required"  name="admin_name"  placeholder="您的姓名" />
						<label for="name">您的密码</label>
						 <input id="txt" type="text"  class="form-control" placeholder="您的密码" onfocus="change(this)"> 
                        <input id="pass" type="password" name="admin_pass"class="form-control" style="display:none" onblur="change(this)"> 
