<?php 
	...
	switch ($a) {
		case 'del':
			$id=$_GET['id'];
			// echo "啊哦，数据被删除了";
			$sql="delete from business_admin where admin_id=$id";
			 // echo $sql;
	    ...
	    //提交修改 update
		    ...
		    $sql="update business_admin set `admin_name`='$admin_name',`admin_pass`='$password' where `admin_id`=$id";
		    // echo $sql;exit;
		    $result=mysql_query($sql);
		    //修改返回提示，并跳转会用户列表首页
		    if($result){
		...
		case 'add':
			...
			$password=@$_POST['password'];
			$email=@$_POST['email'];
			$str="insert business_admin(admin_name,admin_pass) values('$admin_name','$password')";
			// echo $str;
